...
watch(() => fileDataStore.audioArray, (newVal) => {
	...
	newVal.forEach((item, index) => {
		...
		const item_img = fileDataStore.filterFileByType('image').find((o) => {
			return baseName(o.name) === baseName(item.name)
		})

		const item_lrc = fileDataStore.filterFileByType('text').find((o) => {
			return baseName(o.name) === baseName(item.name) && o.name.endsWith(".lrc")
		})

		console.log((item_lrc && item_lrc.url))
